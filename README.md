...
6. The last change is the number of user-cofig files to generate. I will just stick with the defaults.
7. Start Wireguard:
```
cd ~/wireguard
docker-compose up -d
...
## Testing the VPN
### Testing on a Phone
1. Navigate to the App Store (iPhone in my case) and download the Wireguard app. It should be obvious which one it is.
2. Before entering the VPN, go to `ipleak.net` to see your before IP address. Here is mine:
...
6. You are now connected! Go back to `ipleak.net` and look at your new IP. Here is mine:
...
5. Copy the file
6. CTRL + X to exit
7. Open TextEdit and paste the contents.
8. Save somewhere safe. I called mine `peer_pc1.conf`, just as it was in the console.
